...
| 트리거 | 동작                                              | 사용 시점 |
|-------|--------------------------------------------------|---------|
| `/tg` | 단독 명령. bridge 모드 진입.                       | 가장 일반적 |
| `>>tg <메시지>` | 메시지 먼저 Claude로 보내고, 응답부터 bridge. | 마지막 한마디 후 핸드오프 |
| Telegram의 `🔌 Bridge` 버튼 | 봇이 먼저 시작 — Mac이 자동 합류.   | 핸드폰에서 먼저 시작 |

MVP는 `/tg`만 구현. 나머지는 후속.
