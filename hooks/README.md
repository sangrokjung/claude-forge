...
```json
{
  "hooks": {
    "SessionEnd": [
      { "hooks": [ { "type": "command", "command": "~/.claude/hooks/session-time-report.sh", "timeout": 10 } ] }
    ],
    "SessionStart": [
      { "hooks": [ { "type": "command", "command": "FORGE_SESSION_MODE=report ~/.claude/hooks/session-time-report.sh", "timeout": 5 } ] }
    ]
  }
}
```
