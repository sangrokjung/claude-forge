...
| **jq** | macOS/Linux | JSON 파싱 (설치 스크립트) |
| **python3** | 권장 | hooks 내부 JSON 파싱에 사용. 없으면 hooks가 조용히 비활성화됨 |
| **.NET SDK** | .NET 프로젝트 시 | `dotnet` CLI. `install.ps1`이 자동으로 버전을 확인함 |
...
cd ~ && git clone --recurse-submodules https://github.com/daeha76/claude-forge.git
...
`install.ps1`이 수행하는 추가 확인:
- **node**, **git** 필수 의존성 확인
- **.NET SDK** 설치 여부 감지 (있으면 버전 출력, 없으면 설치 안내)
