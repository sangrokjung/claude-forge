...
> 🔁 **v4.3.0 (2026년 9월, 최신)**: **session-time-report가 Codex도 읽어요.** 같은 훅, 같은 리포트인데 Claude Code 기록인지 Codex rollout 로그인지 알아서 구분해서 셉니다. Codex 배선은 `~/.codex/hooks.json`에 두 줄 넣으면 돼요([방법](hooks/README.md#running-a-hook-under-codex)).
>
> ⏱️ **v4.2.0 (2026년 9월)**: **session-time-report**(22번째 훅)를 추가했어요. 세션이 닫힐 때 시간을 재두고, 다음에 세션을 열면 몇 시에 끝났는지, 실제로 얼마나 걸렸는지, 프롬프트와 도구를 얼마나 썼고 파일을 몇 개 고쳤는지 한 줄로 알려줘요. 며칠 뒤에 이어서 연 세션이라도 전체 경과가 아니라 이번에 앉아 있던 시간만 보여줍니다. 기록은 `~/.claude/work-log/`에 남아요.
>
...
Codex도 읽어요. Codex 0.150은 훅 이벤트가 같고 자기 형식의 rollout 로그를 쓰는데, 이 훅이 어느 쪽 기록인지 알아서 판별하고 양쪽 다 세어줍니다. 출력 줄에 어느 에이전트였는지도 같이 나와요. 다만 `install.sh`는 Claude Code만 배선하니, Codex 쪽 두 줄은 직접 넣으셔야 합니다([방법](hooks/README.md#running-a-hook-under-codex)).
...
```
claude-forge/
  ├── .claude-plugin/            플러그인 매니페스트 (4.3.0)
